...
public class Forwarder extends Human implements FaorwarderOpportunities, Serializable {
    ...
    public Forwarder(String name, int cloth){super(name,cloth);}

    @Override
    public int hashCode() {
        return Objects.hash(lastFind); }
    public String toString() {
        return String.format("Экспедитор, имя = %s, одежда = %s, находил картину размером = %s", getName(), getClothes(), getFind().getHeightWidth());
    }
    ...
    public String hope() throws GodException {
        ...
        } return "Надеются, что абстрактый бог номера 5(Братчиков) поможет им сесть";
    }
